			       ...
			       VALUE passed_block_handler, const rb_cref_t *cref)
{
  ...
  again:
    ...
    //   printf("vm.c:1055\n");
    //   struct rb_captured_block *tmp_cap = VM_BH_TO_ISEQ_BLOCK(block_handler);
    //   block_handler += th->diff_sp;
    //   struct rb_captured_block *captured = VM_BH_TO_ISEQ_BLOCK(block_handler);
    //   block_handler -= th->diff_sp;
    //   memcpy(tmp_cap,captured,sizeof(struct rb_captured_block));
    //   return invoke_iseq_block_from_c(th, tmp_cap, tmp_cap->self, argc, argv, passed_block_handler, cref, TRUE, is_lambda);
    // }else{
...
{
  VALUE r;
  //if(th->check == 2) printf("vm.c:1110\n");
    r = invoke_block_from_c_splattable(th, check_block_handler(th), argc, argv, VM_BLOCK_HANDLER_NONE, NULL);
    return r;
}
...
rb_thread_recycle_stack_release(VALUE *stack)
{
    ...
    // add
    th = GET_THREAD();
    if(munmap(stack,th->stack_size) == -1){
      if(stack != NULL){
        free(stack);
        stack = NULL;
      }
...
thread_free(void *ptr)
{
	...
	if (th->vm && th->vm->main_thread == th) {
	    ...
	    if (th->altstack) {
		//free(th->altstack);
    munmap(th->altstack,rb_sigaltstack_size());
	    }
#endif
	    ruby_xfree(ptr);
	}
...
th_init(rb_thread_t *th, VALUE self)
{
    ...
    th->altstack = (void *)mapping(rb_sigaltstack_size());
#endif
    ...
    th->stack = (VALUE *)mapping(th->stack_size);
...
Init_BareVM(void)
{
    ...
    //rb_vm_t *vm = mapping(sizeof(*vm));
    //rb_thread_t * th = ruby_mimmalloc(sizeof(*th));
    rb_thread_t *th = mapping(sizeof(*th));
    if (!vm || !th) {
