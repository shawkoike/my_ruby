...
vm_call0(rb_thread_t* th, VALUE recv, ID id, int argc, const VALUE *argv, const rb_callable_method_entry_t *me)
{
    ...
    r = vm_call0_body(th, calling, &ci_entry, &cc_entry, argv);
...
vm_call0_cfunc(rb_thread_t* th, struct rb_calling_info *calling, const struct rb_call_info *ci, struct rb_call_cache *cc, const VALUE *argv)
{
  ...
  if(th->check == 2 || th->flag_for_captured == 2) reg_cfp = th->cfp + 1;
	 ...
	 call_type scope, VALUE self)
{
    ...
    stack_check(th);
    r = vm_call0(th, recv, mid, argc, argv, me);
...
rb_call(VALUE recv, ID mid, int argc, const VALUE *argv, call_type scope)
{
    rb_thread_t *th = GET_THREAD();
    VALUE r;
    r = rb_call0(recv, mid, argc, argv, scope, th->cfp->self);
    return r;
}
...
rb_funcall(VALUE recv, ID mid, int n, ...)
{
    ...
    r = rb_call(recv, mid, n, argv, CALL_FCALL);
    return r;
}
...
rb_yield_0(int argc, const VALUE * argv)
{
  VALUE r;
  rb_thread_t *th = GET_THREAD();
    r = vm_yield(GET_THREAD(), argc, argv);
  return r;
}
...
rb_yield(VALUE val)
{
  ...
  rb_thread_t *th = GET_THREAD();
	...
	r = rb_yield_1(val);
  //if(th->check == 2) printf("vm_eval.c:1038\n");
  return r;
    }
}
