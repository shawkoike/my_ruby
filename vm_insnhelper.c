...
// add
static void
remapping(void *ptr, size_t new_size, size_t old_size)
{
  void *dummy;
  dummy = mremap(ptr,old_size, new_size, MREMAP_MAYMOVE, MREMAP_FIXED);
  if(dummy == MAP_FAILED){
    vm_stackoverflow();
  }else{
    //printf("remap suc!!!!\n");
...
static int count_cfp(rb_control_frame_t *base_cfp,rb_control_frame_t *cfp){
  int count = 0;
  while(base_cfp != cfp){
    base_cfp--;
    count ++;
                  ...
                  VALUE type,VALUE self,VALUE specval,VALUE cref_or_me,const VALUE *pc)
{
  ...
  if(!VM_STACK_OVERFLOWED_P(cfp, sp, margin)){
    // 成功時（スタックオーバーフロー起こしていない時）
    return cfp;
  }
  else{
    // check trigger for stackoverflow
    if(!RUBY_CONST_ASSERT(sizeof(*(sp)) == sizeof(VALUE))){
      vm_stackoverflow();
    }else if(!RUBY_CONST_ASSERT(sizeof(*(cfp)) == sizeof(rb_control_frame_t))){
      vm_stackoverflow();
    }else if(((rb_control_frame_t *)((sp) + (margin)) + 1) >= (cfp)){
      // declarations
      ...
      printf("base  cfp\n");
      while(cfp_base > cfp){
        printf("block_code: %p            &block_code: %p\n",cfp_base->block_code,&cfp_base->block_code);
        if(stack_start <= (void *)cfp_base->ep && (void *)cfp_base->ep <= stack_end){
          printf("ep        : %p * &ep        : %p\n",cfp_base->ep,&cfp_base->ep);
        }else{
          printf("ep        : %p   &ep        : %p\n",cfp_base->ep,&cfp_base->ep);
        }
        if(stack_start <= (void *)cfp_base->self && (void *)cfp_base->self <= stack_end){
          printf("self      : %p * &self      : %p\n",cfp_base->self,&cfp_base->self);
        }else{
          printf("self      : %p   &self      : %p\n",cfp_base->self,&cfp_base->self);
        }
        if(stack_start <= (void *)cfp_base->iseq && (void *)cfp_base->iseq <= stack_end){
          printf("iseq      : %p * &iseq      : %p\n",cfp_base->iseq,&cfp_base->iseq);
        }else{
          printf("iseq      : %p   &iseq      : %p\n",cfp_base->iseq,&cfp_base->iseq);
        }
        if(stack_start <= (void *)cfp_base->sp && (void *)cfp_base->sp <= stack_end){
          printf("sp        : %p * &sp        : %p\n",cfp_base->sp,&cfp_base->sp);
        }else{
          printf("sp        : %p   &sp        : %p\n",cfp_base->sp,&cfp_base->sp);
        }
        if(stack_start <= (void *)cfp_base->pc && (void *)cfp_base->pc <= stack_end){
          printf("pc        : %p * &pc        : %p\n",cfp_base->pc,&cfp_base->pc);
        }else{
          printf("pc        : %p   &pc        : %p\n",cfp_base->pc,&cfp_base->pc);
        }
        printf("cfp       : %p   &cfp       : %p\n",cfp_base,&cfp_base);
      ...
      cfp_base++;
      tmp_cfp_base = cfp_base;
      tmp_sp_for_print = (void *)(th->stack + th->stack_size);

      while(tmp_sp_for_print >= (void *)th->stack){
        printf("%p\n",tmp_sp_for_print);
        tmp_sp_for_print--;
      }


      cfp_base = tmp_cfp_base;
#endif
      ...
      while(cfp_base > cfp){
        ...
        *cfp_move = *cfp_base;
        ...
        if(cfp_base->sp == NULL){
          cfp_move->sp = (void *)0x0;
        }else{
          while(tmp_sp <= for_finish){
            cfp_move->sp = cfp_base->sp - diff_sp;
            *cfp_move->sp = *cfp_base->sp;
            cfp_base->sp--;
            cfp_move->sp--;
            tmp_sp++;
            j++;
      ...
      sp -= diff_sp;
            ...
            printf("\n\ncfp_move\n");
            while(cfp_base > cfp){
              if(stack_for_mremap <= (void *)cfp_move->block_code && (void *)cfp_move->block_code <= new_stack_end){
                printf("block_code: %p *          &block_code: %p\n",cfp_move->block_code,&cfp_move->block_code);
              }else{
                printf("block_code: %p            &block_code: %p\n",cfp_move->block_code,&cfp_move->block_code);
              }
              if(stack_for_mremap <= (void *)cfp_move->ep && (void *)cfp_move->ep <= new_stack_end){
                printf("ep        : %p * &ep        : %p\n",cfp_move->ep,&cfp_move->ep);
              }else{
                printf("ep        : %p   &ep        : %p\n",cfp_move->ep,&cfp_move->ep);
              }
              if(stack_for_mremap <= (void *)cfp_move->self && (void *)cfp_move->self <= new_stack_end){
                printf("self      : %p * &self      : %p\n",cfp_move->self,&cfp_move->self);
              }
              else{
                printf("self      : %p   &self      : %p\n",cfp_move->self,&cfp_move->self);
              }
              if(stack_for_mremap <= (void *)cfp_move->iseq && (void *)cfp_move->iseq <= new_stack_end){
                printf("iseq      : %p * &iseq      : %p\n",cfp_move->iseq,&cfp_move->iseq);
              }else{
                printf("iseq      : %p   &iseq      : %p\n",cfp_move->iseq,&cfp_move->iseq);
              }
              if(stack_for_mremap <= (void *)cfp_move->sp && (void *)cfp_move->sp <= new_stack_end){
                printf("sp        : %p * &sp        : %p\n",cfp_move->sp,&cfp_move->sp);
              }else{
                printf("sp        : %p   &sp        : %p\n",cfp_move->sp,&cfp_move->sp);
              }
              if(stack_for_mremap <= (void *)cfp_move->pc && (void *)cfp_move->pc <= new_stack_end){
                printf("pc        : %p * &pc        : %p\n",cfp_move->pc,&cfp_move->pc);
              }else{
                printf("pc        : %p   &pc        : %p\n",cfp_move->pc,&cfp_move->pc);
              }
              printf("cfp       : %p   &cfp       : %p\n",cfp_move,&cfp_move);
            ...
            cfp_move++;
            tmp_cfp_move = cfp_move;
            tmp_sp_for_print = (void *)(stack_for_mremap + new_size);

            while(tmp_sp_for_print >= (void *)stack_for_mremap){
              printf("%p\n",tmp_sp_for_print);
              tmp_sp_for_print--;
            }


            cfp_move = tmp_cfp_move;
            fflush(stdout);
#endif

      cfp_move->pc = (VALUE *)pc;
      cfp_move->iseq = (rb_iseq_t *)iseq;
      cfp_move->self = self;
...
{
    int i;
    rb_control_frame_t *cfp = th->cfp - 1;
    rb_control_frame_t *cfp_new;
...
#ifdef mode
  th->check = 0;
  cfp_new = dummy(cfp,sp,local_size+stack_max,th,local_size,iseq,type,self,specval,cref_or_me,pc);
  th->cfp = cfp_new;
#else
    /* check stack overflow */
    CHECK_VM_STACK_OVERFLOW0(cfp, sp, local_size + stack_max,th);
    th->cfp = cfp;
#endif
    ...
    if(th->check == 2){
      ...
      /* setup new frame */
      cfp_new->pc = (VALUE *)pc;
      cfp_new->iseq = (rb_iseq_t *)iseq;
      cfp_new->self = self;
      cfp_new->block_code = NULL;
      /* initialize local variables */
...
rb_equal_opt(VALUE obj1, VALUE obj2)
{
    ...
    cc.class_serial = 0;
    cc.me = NULL;
    val = opt_eq_func(obj1, obj2, &ci, &cc);
    return val;
}
...
// add
static VALUE *
vm_new_base_ptr(rb_thread_t *th,rb_control_frame_t *cfp)
{
    const rb_control_frame_t *prev_cfp = RUBY_VM_PREVIOUS_CONTROL_FRAME(cfp);
...
vm_call_iseq_setup(rb_thread_t *th, rb_control_frame_t *cfp, struct rb_calling_info *calling, const struct rb_call_info *ci, struct rb_call_cache *cc)
{
  VALUE v;
    ...
    v = vm_call_iseq_setup_2(th, cfp, calling, ci, cc, opt_pc, param_size, local_size);
    return v;;
}
...
vm_call_cfunc(rb_thread_t *th, rb_control_frame_t *reg_cfp, struct rb_calling_info *calling, const struct rb_call_info *ci, struct rb_call_cache *cc)
{
  VALUE val;
    CALLER_SETUP_ARG(reg_cfp, calling, ci);
    val = vm_call_cfunc_with_frame(th, reg_cfp, calling, ci, cc);
    return val;
}
#endif
...
vm_call_zsuper(rb_thread_t *th, rb_control_frame_t *cfp, struct rb_calling_info *calling, const struct rb_call_info *ci, struct rb_call_cache *cc, VALUE klass)
{
  VALUE val;
    ...
    if (cc->me != NULL) {
	val = vm_call_method_each_type(th, cfp, calling, ci, cc);
...
{
  VALUE val;
    val = vm_call_method(th, reg_cfp, calling, ci, cc);
    return val;
}
